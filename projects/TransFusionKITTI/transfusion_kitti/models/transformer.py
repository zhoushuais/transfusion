...
# Adapted for the standalone TransFusion KITTI project.
...
class TransFusionKITTITransformerDecoderLayer(DetrTransformerDecoderLayer):
    ...
    def forward(self,
        ...
        if not self.cross_only:
            # TransFusion encodes the self-attention value with query_pos.
            query = self.self_attn(
                query=query,
                key=query,
                value=query if query_pos is None else query + query_pos,
                query_pos=query_pos,
                key_pos=query_pos,
        ...
        query = self.cross_attn(
            query=query,
            key=key,
            value=key if key_pos is None else key + key_pos,
            query_pos=query_pos,
            key_pos=key_pos,
